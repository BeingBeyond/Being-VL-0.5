...
# Being-VL Stage 2 Training: Selective Fine-tuning
# Purpose: Enhance cross-modal interactions in early transformer layers
# Strategy: Unfreeze early layers, use SFT data with balanced composition
...
if [ $# -eq 0 ]; then
    ...
    echo "Usage: $0 <master_node_id> [stage1_model_path]"
    exit 1
fi
MASTER=$1

# Stage 1 model path (can be provided as second argument)
if [ $# -ge 2 ]; then
    STAGE1_MODEL_PATH="$2"
else
    # Default path - consistent workspace structure
    STAGE1_MODEL_PATH="/path/to/your/workspace/models/beingvl/stage-1/final"
fi
...
# Stage 2 Model Path - Should be the output from Stage 1
MODEL_PATH="$STAGE1_MODEL_PATH"

# Stage 2 Data - SFT data with balanced composition
# Data composition: Foundation Data 40% + Perception Data 30% + Reasoning Data 20% + Instruction Data 10%
...
FREEZE_TEXT_HEADS="False"    # Unfreeze for selective fine-tuning
...
echo "Stage 2 training script backed up to: $OUTPUT_DIR"

echo "========================================="
echo "Being-VL Stage 2 Training: Selective Fine-tuning"
...
if [ ! -d "$MODEL_PATH" ]; then
    echo "Error: Stage 1 model not found at $MODEL_PATH"
...
echo "Stage 2 training completed. Model saved to: $OUTPUT_DIR/final"
echo "Next: Run train-stage-3.sh with MODEL_PATH=$OUTPUT_DIR/final"